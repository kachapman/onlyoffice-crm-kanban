...
class KanbanHandler(SimpleHTTPRequestHandler):
    ...
    def _current_user_email(self, portal: str, token: str) -> str:
        """Best-effort resolve the current user's email from CRM."""
        try:
            url = f"{portal.rstrip('/')}/api/2.0/people/@self.json"
            req = urllib.request.Request(
                url,
    ...
    def _handle_event_log_admin_get(self) -> None:
        ...
        target_user = (query.get("userId") or [""])[0]
        if not target_user:
    ...
    def _handle_health_check(self) -> None:
        """Discrete CRM reachability check. Does not poll; only called on demand."""
        portal = _portal_base(self)
        if not portal:
        ...
        try:
            ...
            _json_response(self, 200, {
                "ok": False,
                "crmReachable": False,
                "error": str(e) or "CRM unreachable",
                "checkedAt": datetime.now(timezone.utc).isoformat(),
            })
