...
- **Fixed squared-off artifacts on tile corners.** Resize handles (22x22px grip squares) were extending past the tile's `border-radius` because `.dashboard-tile` uses `overflow:visible`. Added `overflow:hidden` to `.tile-resize-handle` and matching `border-radius` on SE (`0 0 var(--radius) 0`) and SW (`0 0 0 var(--radius)`) handles so grip lines clip cleanly to the tile's rounded corners.
- **Minimized tiles now uniform height.** Notes tiles had extra toolbar items (File/Edit menus, formatting buttons B/I/S/U/H/emoji, accent picker, mode toggle) that remained visible when collapsed, making them taller than other minimized tiles. Added CSS rules to hide `.notes-quick-format-group`, `.tile-menu-wrap`, and `.btn-notes-mode-toggle` when `.notes-tile.tile-body-collapsed`. Same treatment for calendar tiles: `.calendar-month-nav`, `.calendar-tz-label`, `.calendar-tz-select` hidden when collapsed. All minimized tiles now show only the title row + actions.
- Cache-bust bumped to `styles.css?v=1.87.16`.
