...
- **Fixed visible gaps between tiles in the grid.** Root cause: `.dashboard-tiles` had `align-items: start` which prevented shorter tiles from stretching to fill their row height. When one tile in a row was tall (e.g. Team), shorter tiles (Tasks, Notifications) left large empty gaps below them. Removed `align-items: start` so tiles use the default `stretch`, filling the entire grid row height. Collapsed tiles already override with `align-self: start`.
