...
**Last session summary (for next resume):** 2026-07-21 session 4: Phase 2D-3 group resize + notes toolbar title-row restructure. Replaced 90 lines of hardcoded .tile-quarter/.tile-half order rules with single CSS container query (@container max-width:550px). Actions always far right via margin-left:auto. Fixed header logo overlapping buttons at narrow widths (@media max-width:1050px moves logo to top-left at 48px). New 840px breakpoint: hero-actions wraps to row 2 before overlap. Full tile resize audit: require 30px min Y delta before height decision (prevents accidental double). Team tile: added applyTileLayoutClasses. All board-group-tile .board: min-height 0 (was 420px). Local kanban: padding-bottom 0. Panels: double-height min-height override. Footer: removed hover + logo. **Fixed erratic resize:** bindTileResize now uses actual rendered tile height (singleRowH/baseHeight) for ghost preview and height boundary instead of CSS var --tile-row-height which didn't match actual row heights. **Reverted align-items:start** — tiles keep natural height; double-height groups no longer pull neighbors up. **Panel tiles fixed:** feed-list and tasks-by-user now flex:1 to fill tile (no empty space below bordered box). **Group tile height capped:** .board max-height + overflow-y:auto so columns scroll instead of pushing tile to unbounded height. **Team tile toolbar consolidated:** status dropdown, tabs, admin button moved from panel-header into tile-toolbar (tools/actions). **Calendar chunked:** renderCalendarMonthBody renders 1 week per requestIdleCallback; loadCalendarForTile render deferred via rAF. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
