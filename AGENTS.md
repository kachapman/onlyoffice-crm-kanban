...
**Last session summary (for next resume):** 2026-07-22 session 7: Phase 2D-3 spikey tile corner artifacts — root-cause fix. Reverted the fe4ccf8 removal of `.dashboard-tile.board-group-tile { overflow: hidden; }`, which had exposed square-corner artifacts at tile corners. Restored the rule plus `padding-bottom: 1rem` on `.board-group-tile .board` (and `.local-kanban-tile .board`) so the horizontal scrollbar sits above the rounded bottom corners and isn't clipped. Also fixed preview modal tag add dropdown: when no tags exist, the + button is at the modal's left edge and `.template-dropdown`'s default `right:0` was opening the menu off-screen to the left; added `.opp-preview-tag-add-wrap .template-dropdown { left: 0; right: auto; }` so it opens to the right. Keeps resize handle shrink and uniform minimized-height fixes from sessions 6–7. Cache-bust: styles.css?v=1.87.19. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
