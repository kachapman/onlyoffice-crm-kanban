...
**Last session summary (for next resume):** 2026-07-22 session 8: Team popup Messages tab fixes. Added `min-height: 320px` to `.presence-list` so the Messages tab no longer shrinks the modal when the inbox is empty. When no recent DMs exist, the Messages tab now falls back to the team roster with a "Select a team member to start messaging" header, letting users open a DM thread directly from the Messages tab. Also retains session 7 fixes: restored `.board-group-tile { overflow: hidden; }` with `padding-bottom: 1rem` on boards to fix spikey corner artifacts while keeping horizontal scrollbars visible; preview tag add dropdown now opens to the right. Cache-bust: app.js?v=1.95.8, styles.css?v=1.87.20. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
