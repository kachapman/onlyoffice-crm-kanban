...
**Current version:** 1.91.0 (released 2026-06-24; see CHANGELOG.md)  
**Last session summary (for next resume):** v1.91.0 shipped: event-note GUID validation, removal of auto-close timers on note modals, server-side 7-day event-log persistence, admin event-log tab for `kenc@vanguardadj.com`, and a discrete `/api/health` CRM check. All pushed to GitHub. Old DNS records for `office.vanguardadj.com` and `dashboard.vanguardadj.com` were already removed during migration cleanup.
