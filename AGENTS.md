...
**Last session summary (for next resume):** 2026-07-21 session: Phase 2D-2 — Dashboard overhaul attempted but STILL BROKEN. See KNOWN ISSUES below for full audit. Key problems: (1) `grid-auto-flow: dense` causes tiles to jump around during resize/SortableJS drag. (2) Resize saves to localStorage on EVERY mousemove (no debounce). (3) SortableJS `forceFallback` clone conflicts with CSS Grid. (4) No ghost/preview during resize — tiles jump instantly. (5) `will-change: transform` on every tile wastes GPU. Cache-bust v=1.95.4. Committed but NOT pushed — DO NOT PUSH until dashboard is verified working.
...
- **KNOWN: Dashboard tiles jump around erratically during resize and drag.** Root causes: (1) `grid-auto-flow: dense` (styles.css:1352) causes cascading reflow when any tile changes span. (2) Resize saves to localStorage on EVERY mousemove (app.js:1491 → 830 → 762). (3) SortableJS `forceFallback` clone conflicts with CSS Grid layout. (4) No ghost/preview during resize — tiles jump instantly. (5) `will-change: transform` on every tile wastes GPU. (6) Column width calculation uses moving target during dense reflow. (7) Full DOM teardown in `mountDashboardTiles()` destroys all event listeners.
- **KNOWN: Odysseus reference architecture differs from current implementation.** Odysseus uses `position: fixed` windows with snap zones (no CSS Grid). Sietch uses CSS Grid which makes smooth resizing difficult because Grid reflows all items when any item changes. The user wants Sietch to work more like Odysseus but the architectures are fundamentally different. See https://github.com/odysseus-dev/odysseus/blob/dev/static/js/tileManager.js for reference.
