...
**Last session summary (for next resume):** 2026-07-22 session 16: Rebranded active codebase and docs from "Vanguard" to "Sietch CRM". Updated user-facing strings in backend/frontend files and active infrastructure docs to use `sietch-crm` container name and `dashboard.publicadjustermidwest.com` production domain. Removed hardcoded admin credential references (`kenc@vanguardadj.com`, `bot@vanguardadj.com`, `FRi3tz4yWXrMTEZ`); `BOT_CRM_EMAIL`/`BOT_CRM_PASSWORD` now obsolete. Added photo batch actions (download, move, copy, delete) via a single Actions dropdown matching the Documents modal, plus ZIP endpoint, move/copy/delete endpoints. Also unified the Documents tab in the preview modal with the Documents-modal list UI: Actions dropdown, select-all checkbox, file-type icons, size/date columns, and hover download button. Redesigned admin console sidebar toggle: SVG chevron, absolutely positioned at boundary (replaces divider), works on all screen sizes, soft dark-mode color. Improved Document Server self-signed-cert UX in `doc-editor.html` with a clickable healthcheck link. Left historical release notes and migration runbooks untouched. Ran Python/JS/compose syntax checks and restarted local server; `/api/config` returns `version: 2.2.5`. PUSHED.
