...
**Last session summary (for next resume):** Phase 1A-1D foundation: PostgreSQL schema (34 tables), db.py connection layer, auth.py (PBKDF2 + sessions), smtp_client.py, migrate_from_onlyoffice.py, server.py rewrite (v2 API). Phase 1E: Frontend API path swaps (/api/2.0/crm/* → /api/v2/*), dead code removal (~500 lines: mutation queue, crash banner, mail system), body format updates. Document Server decision: Keep OnlyOffice Document Server for document editing (link-out initially, embedded later). README.md rewritten as Sietch CRM with Dune/Sietch branding. CHANGELOG.md updated with v3.0.0 entry. Phase 1G: Telegram notification dispatch + inline replies. notification_dispatcher.py background worker sends notifications when users are tagged in notes or assigned tasks. /api/bot/send-message endpoint implemented (was 501 stub). Reply handler in telegram_bot.py posts threaded notes on projects. Added telegram_notification_log table, create_task_notifications trigger, userId field to employee mappings. Logo added to public/assets/sietch-logo.png.
