...
**Last session summary (for next resume):** 2026-07-22 session 9: Team tile Messages tab real fixes. Root cause of "No messages yet" despite existing messages: (1) `/api/presence` endpoint was not returning `myRecentDms` or `lastReadDms`; (2) `renderPresenceTileCompact` only re-rendered the Team tab body on snapshot updates, so the Messages tab never refreshed. Updated `server.py` `_handle_presence_get` to include both fields, and added a refresh of `#presence-tile-messages-body` inside `renderPresenceTileCompact` when the Messages tab is visible. Fixed tile shrinking: removed `max-height: 280px` from `.dashboard-tile.presence-panel .presence-tile-body` and set `min-height: 240px`. Server restarted to pick up the new code. Also retains session 8 popup modal fallback and session 7 fixes. Cache-bust: app.js?v=1.95.10, styles.css?v=1.87.21. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
