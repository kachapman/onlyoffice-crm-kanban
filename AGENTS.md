...
**Last session summary (for next resume):** 2026-07-22 session 9: Team tile Messages tab real fixes. Root cause of "No messages yet" despite existing messages: `/api/presence` endpoint was not returning `myRecentDms` or `lastReadDms`. Updated `server.py` `_handle_presence_get` to include both. Fixed tile shrinking: removed `max-height: 280px` from `.dashboard-tile.presence-panel .presence-tile-body` and set `min-height: 240px` so the body fills the tile instead of collapsing when empty. Also retains session 8 popup modal fallback (team roster in empty Messages popup) and session 7 fixes (board-group-tile overflow + padding, preview tag dropdown right-open). Cache-bust: app.js?v=1.95.9, styles.css?v=1.87.21. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
