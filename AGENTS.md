...
**Current version:** 1.2.0 (released 2026-06-26; see CHANGELOG.md)  
**Last session summary (for next resume):** v1.2.0 Telegram customer bot release: (1) `telegram_bot.py` — async Telegram bot (@vanguardupdates_bot) using python-telegram-bot v22.8, invite-code-based customer linking, deal/project status queries with summary + detail views. (2) `crm_bot_store.py` — JSON persistence for mappings and pending invite codes (48h expiry). (3) 7 server endpoints: list mappings, generate/cancel/verify code, check mapping, fetch deals, check admin. (4) Bot CRM session management (`_bot_crm_token`, `_bot_crm_proxy`). (5) Bot Customers admin modal with searchable contact picker, note category select, code generation with Copy + expiry + Cancel, mapping list with linked/pending badges and × unlink. (6) Admin detection extended (CRM `isAdmin` + email fallback). (7) Bot button positioned next to event-log button in header. (8) All customer-facing text uses "projects" instead of "deals". (9) MAX_BOOKMARKED_DEALS increased to 20. See CHANGELOG.md and docs/RELEASE_v1.2.md.
