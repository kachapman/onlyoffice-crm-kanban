...
**Current version:** 2.2.2 (released 2026-07-13; see CHANGELOG.md)  
**Last session summary (for next resume):** v2.2.2 hotfix: Login screen — removed duplicate "Logging in…" paragraph, updated portal URL to publicadjustermidwest.com. Email history — added `/api/mail/message/{id}` server endpoint using `filehandler.ashx` (CRM REST API was returning 404); uses bot token for filehandler (user tokens don't have access), falls back to REST API with user token then bot token; client `fetchMailMessage` now calls this. Regex crash — replaced regex-based escaping with string methods (`startsWith`, `endsWith`, `includes`) to avoid regex compilation entirely. Custom field character limit doubled (`parseCustomFieldTextMaxLength` returns `size * 2`). Attachment links in email history notes now open in DocEditor instead of downloading. Bot modal — two-column desktop layout (960px), expandable per-user activity log. Telegram bot — customer view shows Address/Phone/Carrier/Claim# below title; employee Project Info button shows all CRM fields + all user fields inline (no header). `_extract_bot_user_fields` now returns ALL custom fields; `_handle_bot_deals` passes additional CRM fields (description, contact, responsible, created, dueDate, bidType, isPrivate). Prior: v2.2.1 tag cache fix.
