...
**Current version:** 2.0.7 (released 2026-06-30; see CHANGELOG.md)  
**Last session summary (for next resume):** 2.0.7 hotfix — preview modal pasted email content now renders for Email-category notes (previously collapsed to "An email has been received." summary); presence/Team tile no longer shows UUID display names when CRM user record lacks displayName/userName (falls back to email, firstName+lastName). See CHANGELOG.md and docs/RELEASE_v2.0.7.md. Prior: 2.0.0 — Telegram customer bot release (`telegram_bot.py`, `crm_bot_store.py`, systemd service `crm-telegram-bot`, venv at `/tmp/botenv`). 2.0.1 hotfix — nickname field for bot mappings, HTML bot messages, simplified project summary, mobile UI fixes (bot/event-log buttons stacked below sign-out, opp preview modal width), presence reliability (stale autoStatus stripped for offline users, team roster kept on poll failure), crash banner 15s grace after tab resume. 2.0.2 hotfix — stale autoStatus also stripped in Team tile (`renderPresenceTileCompact`), bot button swapped to rightmost on mobile. 2.0.3 hotfix — bot search-by-title flow (`?search=` param on `/api/bot/deals`, no more full list, numbered drill-down, `/help` command), modal renamed to "Customer Bot", mobile bot button CSS source-order fix. 2.0.4 — **employee access mode** for the Telegram bot plus bot robustness fixes: admins generate employee invite codes (no contact linked), employees search all open deals and see up to 5 history events per deal with category labels; number replies now use cached results; history API uses `sortBy=created` with 10s timeout; CRM note HTML sanitized for Telegram (`<br>` to newlines, entities decoded, tags balanced, `&` escaped). Backend (`crm_bot_store.py`, `server.py`), bot (`telegram_bot.py`), and dashboard UI (`public/index.html`, `app.js`, `styles.css`) all updated. 2.0.5 hotfix — employee mode mail body improvements: server fetches full email bodies via CRM `filehandler.ashx?action=mailmessage&message_id={id}`, bot converts HTML to tight plain text, escapes email addresses in forward/reply attributions, caps bodies at 1200 chars with `[truncated]` marker, shows event author for non-mail/non-customer-update events, hides author for `Customer Update` events, HTML-safe message truncation, plain-text fallback strips tags, distinct footer. 2.0.6 hotfix — preview modal highlight readability fix for `<b style="background-color:...">` tags, bot modal delete-by-`chatId` fix, Customer Update/Text/SMS delete buttons restored in preview modal, customer-mode latest update author hidden. See CHANGELOG.md and ISSUES.md for API gotchas.
...
- **Production shared-hosting note:** The dashboard droplet also runs other web apps (`estimate-enhancer`, `iws-calculator`). All public traffic enters through a single Docker nginx container (`estimate-nginx`) in `/opt/estimate-enhancer`. The dashboard is in a separate Compose project (`/opt/vanguard/onlyoffice-crm-kanban`) but joins the same Docker network. **Never modify `/opt/estimate-enhancer/nginx.conf` without reading `docs/DASHBOARD_INFRASTRUCTURE.md` first.**
