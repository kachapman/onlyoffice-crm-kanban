...
**Last session summary (for next resume):** 2026-07-22 session 6: Phase 2D-3 corner artifacts + uniform minimized height (follow-up). **Resize handles fixed:** shrunk from 22x22px to 12x12px, moved 6px inward from corners, added pointer-events:none so handles sit inside border-radius zone and are truly non-interactive. Mobile handles: 18x18px. **Uniform minimized height:** Team tile: status dropdown stays visible when collapsed, tabs hidden, panel-header padding normalized to 0.45rem 0.65rem (was 0.65rem 1rem 0 + 0.5rem margin). Open Pipeline: filter summary hidden when collapsed. Notes tiles: explicit padding override when collapsed. Cache-bust: styles.css?v=1.87.17. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
