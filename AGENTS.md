...
**Last session summary (for next resume):** 2026-07-22 session 10: Tasks tile user filter moved into title bar. Removed the separate `.panel-header` row containing "User" label + `<select id="tasks-user-filter">`; the select now lives in `.tile-toolbar-tools` via new `ensureTasksUserFilter()` helper, with compact `.tasks-user-filter-select` CSS so it doesn't thicken the toolbar. Also retains session 9 fixes (Team tile Messages tab server + refresh fixes), session 8 popup fallback, and session 7 fixes. Cache-bust: app.js?v=1.95.11, styles.css?v=1.87.22. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
