...
**Current version:** 1.8.1 (see CHANGELOG.md)  
**Last session summary (for next resume):** v1.8.1 (committed): Multiple fixes: (1) Deal caching — refreshAll() and refreshGroup({force:true}) now clear stale IndexedDB data, server.py invalidates single-opp cache on tag changes. (2) Email width — expanded emails in mail inbox and preview now fill available width. (3) UI freeze — IntersectionObserver batching + visibilitychange pause prevents tab-return freeze. (4) Attachment note indicator — queue status shown in toast when CRM is down. (5) Quote font — DM Sans + italic, larger size. (6) Preview modal stale data — manual refresh button bypasses 30s server proxy cache via ?_t=Date.now(). (7) Amber border lost after deal edit — enrichOpportunitiesTags() called after fetchOpportunityForUpdate() so card styling is preserved. (8) Changelog popup — shows once per version on first login, renders CHANGELOG.md via renderBasicMarkdown(), persists seen version in localStorage. All changes committed as v1.8.1.
