...
**Last session summary (for next resume):** v2.2.2 hotfix: Login screen — removed duplicate "Logging in…" paragraph, updated portal URL to publicadjustermidwest.com. Email history — fixed regex matching (was checking `/api/mail/message/...` but client sends `/api/proxy/api/mail/message/...`); reverted to user token for `filehandler.ashx` (linked emails accessible to all deal users). Regex crash — replaced regex-based escaping with string methods. Custom field character limit doubled. Attachment links in email history notes now open in DocEditor. Bot modal — two-column desktop layout (960px), expandable per-user activity log. Telegram bot — customer view shows Address/Phone/Carrier/Claim# below title; employee Project Info button shows all CRM fields + all user fields inline. `_extract_bot_user_fields` now returns ALL custom fields; `_handle_bot_deals` passes additional CRM fields. Bot linking — contacts can now have up to 10 linked chats (was 1-to-1). `verify_code()` and `add_mapping()` enforce limit; `set_verify_chat_id()` finds correct pending mapping. Prior: v2.2.1 tag cache fix.
