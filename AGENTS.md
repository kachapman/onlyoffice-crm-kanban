...
**Last session summary (for next resume):** 2026-07-21 session 5: Phase 2D-3 corner artifacts + uniform minimized height. **Fixed squared-off corner artifacts:** resize handles (22x22px grip squares) clipped to tile border-radius via overflow:hidden + matching border-radius on SE/SW handles. **Minimized tiles uniform:** notes tiles had extra toolbar items (File/Edit menus, formatGroup B/I/S/U/H/emoji, accent picker, mode toggle) that stayed visible when collapsed — now hidden via CSS. Calendar tiles: month nav + timezone hidden when collapsed. All minimized tiles now show only title row + actions. Cache-bust: styles.css?v=1.87.16. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
