...
**Current version:** 1.91.5 (released 2026-06-27; see CHANGELOG.md)  
**Last session summary (for next resume):** Post-v1.2 changes: (1) FEAT-025 — nickname field on bot customer mappings (text input on generate-code, shown in list with ✎ inline edit, `PUT /api/bot-customers/nickname` endpoint, stored in `crm_bot_store.py` mappings + pending codes). (2) Bot messages now use `parse_mode="HTML"` — CRM note content renders as rich text (bold, italic); plain-text fields escaped via `html.escape()`. (3) Project summary list simplified — no latest note preview, no amount (only title + status). (4) `Dockerfile` fixed to include `crm_bot_store.py` in COPY (was missing → container crash → nginx 502). (5) Bot runs as systemd service (`crm-telegram-bot`) on production, venv at `/tmp/botenv`. (6) Deploy docs updated: .env update must precede docker compose build, bot setup steps, 503 troubleshooting. (7) `.gitignore` updated. (8) **Mobile dashboard UI fixes (3):** bot/event-log buttons stacked below sign-out (Option A); opp preview modal fills full screen width; presence stale autoStatus no longer leaks for offline users in both modal and Team tile + team roster no longer goes blank on poll failure (keeps last good snapshot). (9) Crash banner 15s grace period after tab resume to suppress false positives from browser throttle. See CHANGELOG.md for full log.
...
- **Mobile fixes (3):** bot/event-log buttons stacked below sign-out (Option A); opp preview modal fills screen width; presence stale autoStatus leak fixed + poll failure no longer wipes team roster. See CHANGELOG.md.
