...
**Last session summary (for next resume):** Created `sietch-crm-plan.md` as the single source of truth for migration phases, confirmed decisions (branch `new-crm`, card title click opens preview, one JSON per entity type for export, bidirectional hourly sync, Document Server for future docs). Active work: Phase 1 follow-up UI/JS fixes (kanban stage/stageType field paths, defensive localeCompare, remove preview button, move POST /api/branding out of GET handler, active-user filter for Team tile). Previous: Document Server integration, migration fixes, CSV import, dashboard-local data migration, JS syntax fixes, project-list index fix. See `sietch-crm-plan.md` for current progress.
