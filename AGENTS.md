...
**Last session summary (for next resume):** CRM and dashboard droplet cutover to publicadjustermidwest.com is complete. `office.publicadjustermidwest.com` and `dashboard.publicadjustermidwest.com` both serve HTTPS with valid certificates. Dashboard container is rebuilt on main, `.env` points to the new CRM URL, and user-profile/presence/message data was migrated inside the Docker volume from `https_office.vanguardadj.com` to `https_office.publicadjustermidwest.com`. End-to-end curl checks pass (`/api/proxy/2.0/crm/opportunity/filter.json` returns "Not authenticated" as expected without a session). Next tasks: browser login verification on the new dashboard domain, confirm the CRM "Not secure" browser warning is resolved (likely mixed content or local trust store), then post-cutover cleanup: remove old DNS records after 24–48h and delete old `*vanguardadj*` data directories from the dashboard Docker volume. Ubuntu OS updates on CRM server remain postponed until migration is stable.
