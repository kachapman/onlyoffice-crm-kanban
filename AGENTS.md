...
**Last session summary (for next resume):** Phase 1F/2E: Document Server integration. Added docserver + redis services to docker-compose.yml; DOCS_* env vars to config.example.env; project_documents table to init.sql. Implemented /api/v2/projects/{id}/documents, /api/v2/documents/{id}* (download, editor-config, callback), and OnlyOffice save callback in server.py. Added minimal multipart parser, HS256 JWT signing, and Document Server proxy helpers. Linked uploaded documents to history events via fileIds. Updated app.js to use v2 document endpoints for preview, attachment uploads, and downloads. Fixed migrate_from_onlyoffice.py: event_id now fetched from RETURNING, file owner mapped to v3 uploader, mime_type preserved, file downloads retried on transient errors. Local deployment verified via podman-compose on this machine (Docker unavailable; used Podman). Fixed Dockerfile to install httpx and copy notification_dispatcher.py + telegram_bot.py. Switched docker-compose image names to fully qualified docker.io/... for Podman compatibility. Exposed dashboard on 0.0.0.0:8766 for LAN access and DB on 127.0.0.1:5432 for migration script. Added dashboard-local data migration tooling: users.external_user_id column, CRM migration stores old IDs, new migrate_dashboard_data.py maps old user IDs to new PostgreSQL IDs for user-profiles, dashboard-notes, user-presence, event-logs, presence-messages, and bot-customers mappings. Fixed JS syntax error in public/app.js (orphaned code after uploadAttachmentForNote refactor). Added import_csv_export.py to import OnlyOffice CRM CSV export (contacts, opportunities, tasks, history) and tested it on the user's CRM_export folder. Fixed off-by-one index bug in /api/v2/projects list (stage title was showing color). Commit 7f28153 pushed to new-crm branch.
