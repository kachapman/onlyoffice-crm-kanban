...
**Last session summary (for next resume):** 2026-07-22 session 7: Phase 2D-3 spikey tile corner artifacts — root-cause fix. Reverted the fe4ccf8 removal of `.dashboard-tile.board-group-tile { overflow: hidden; }`, which had exposed square-corner artifacts at tile corners. Restored the rule plus `padding-bottom: 1rem` on `.board-group-tile .board` (and `.local-kanban-tile .board`) so the horizontal scrollbar sits above the rounded bottom corners and isn't clipped. Also keeps resize handle shrink (12x12px, 6px inset, pointer-events:none) and uniform minimized-height fixes from session 6. Cache-bust: styles.css?v=1.87.18. Committed but NOT pushed — DO NOT PUSH until user verifies in browser.
